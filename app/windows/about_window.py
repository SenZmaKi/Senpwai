...
from shared.global_vars_and_funcs import about_bckg_image_path, set_minimum_size_policy, sen_icon_path, morbius_is_peak_icon_path, github_repo_url
...
from shared.shared_classes_and_widgets import StyledLabel, IconButton, ScrollableSection
...
class AboutWindow(Window):
    def __init__(self, main_window: MainWindow):
        ...
        sen_review = Review(sen_icon_path, "SenZmaKi", "69/10 Truly one of the apps of all time.")
        hentai_addict_review = Review(hentai_addict_icon_path, "HentaiAddict01", "0/10 Can't even batch download hentai.. .")
        morbius_is_peak_review = Review(morbius_is_peak_icon_path, "MorbiusIsPeak", "4/10 Morbius better + ratio + morbiusless")
        ...
        donations_label.setText("Konichiwa 👋🏿 it's Sen, the creator of Senpwai, I'm a goofy ahh ahh College Student from Kenya.\nWifi is kinda expensive in my country, so donations help me pay for internet.")
        set_minimum_size_policy(donations_label)
        github_sponsors_button = IconButton(120, 120, github_sponsors_icon_path, 1.1)
        github_sponsors_button.clicked.connect(lambda: open_new_tab("https://github.com/sponsors/SenZmaKi")) # type: ignore
        patreon_button = IconButton(80, 80, patreon_icon_path, 1.1)
        patreon_button.clicked.connect(lambda: open_new_tab("https://patreon.com/Senpwai")) # type: ignore
        ...
        leave_a_star_label.setText("You can also support Senpwai by leaving a star on the github repo, stars help other weebs like us know about it.")
        ...
        bug_reports_label.setText("Found a bug or wanna make a feature request? Report it in the github issues, subreddit or discord server.")
        set_minimum_size_policy(bug_reports_label)
        github_button = IconButton(200, 80, github_icon_path, 1.1)
        github_button.clicked.connect(lambda: open_new_tab(github_repo_url)) # type: ignore
        reddit_button = IconButton(80, 80, reddit_icon_path, 1.1)
        reddit_button.clicked.connect(lambda: open_new_tab("https://reddit.com/r/Senpwai")) # type: ignore
        discord_button = IconButton(80, 80, discord_icon_path, 1.1)
        discord_button.clicked.connect(lambda: open_new_tab("https://discord.gg/e9UxkuyDX2")) # type: ignore
...
class Review(QWidget):
    def __init__(self, icon_path: str, author: str, text: str):
        super().__init__()
        main_layout = QVBoxLayout()
